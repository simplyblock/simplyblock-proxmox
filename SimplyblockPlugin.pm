...
# The API refers to related resources by URL rather than by bare id. Return the
# id such a reference points to, i.e. its last path segment.
#
# References are nullable, so an undefined reference yields undef rather than an
# error. Anything else that does not end in an id is a response we do not
# understand, and is reported instead of silently comparing unequal.
sub _id_from_url {
    my ($url) = validate_pos(@_, 0);
    ...
    die "Malformed resource reference: $url\n"
        unless defined($id) && $id =~ m{\A$UUID_PATTERN\z};
...
sub free_image {
    ...
    my $snapshots = _pool_request($scfg, undef, "GET", "snapshots/")
        or die "Failed to list snapshots\n";
    my $cloned_from = _id_from_url($lvol->{cloned_from});
    foreach (@$snapshots) {
        my $snapshot_lvol = _id_from_url($_->{lvol});
        next unless defined($snapshot_lvol) && $snapshot_lvol eq $lvol->{id};
        next unless defined($cloned_from) && $_->{id} eq $cloned_from;
